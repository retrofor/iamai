...
class RouterPlugin(Plugin):
    ...
    def _route_rules(self) -> list[RouteRule]:
        ...
        config_file = self.config_obj.route_rules_file if self.config_obj is not None else "data/route_rules.json"
    ...
    def _route(self, text: str) -> RouteDecision:
        ...
        return RouteDecision(source="heuristic", tool_name="echo", tool_input=clean, reason="fallback reply", score=0.1)
    ...
    async def _turn(self, ctx: Context, text: str, *, origin: str) -> str:
        ...
        trace = TraceRecord(
            ...
            source_signature=decision.skill_id or f"{decision.tool_name}:{slugify(clean, fallback='input')}",
            path=[f"{decision.source}:{decision.skill_id or decision.tool_name}", f"tool:{decision.tool_name}"],
        )
        try:
            ...
            if origin != "inspect" and bool(skills.config.get("auto_promote", True)):
                ...
                if generated is not None:
                    logger.info("auto skill promoted skill_id=%s source_trace=%s", generated.id, trace.trace_id)
            return reply
