    ...
    def _init_middlewares(self) -> None:
        ...
        try:
            if middleware_name == 'onebot11':
                from .middleware.onebot11 import OneBot11Middleware, OneBot11MiddlewareConfig
                
                if isinstance(middleware_config_data, dict):
                    middleware_config = OneBot11MiddlewareConfig(**middleware_config_data)
                else:
                    middleware_config = middleware_config_data
                
                middleware = OneBot11Middleware(middleware_name, middleware_config, self)
