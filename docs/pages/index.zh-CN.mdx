...
<div className="home-content">
  <div className="content-container">
    <h1 className="headline">机器学习+<br className='sm:block hidden'/> 跨平台机器人框架。</h1>
    <p className="subtitle">适配 <br className='sm:block hidden'/>transformers<a className="huggingface-link" href="https://huggingface.co" target="_blank">(了解Transformer)</a>。</p>
    <p className="subtitle">
      <Link className={styles.cta} href="/docs">快速开始<span>→</span></Link>
    </p>
  </div>
  <style jsx>{`
    .content-container {
  ...
  <div className="features-container">
    <div className="content-container">
      <Features>
        <Feature index={0} large centered id="docs-card" href="/docs/getting-start">
          <Image src={docsCard} alt="Background" loading="eager" />
          <Image src={docsCardDark} alt="Background (Dark)" loading="eager" />
          <h3>2分钟内 <br className="show-on-mobile"/>快速构建跨平台机器人</h3>
        </Feature>
        <Feature index={1} centered className="card-with-border" href="/docs/guide/image">
          <div className="h-full flex flex-col justify-between">
            <h3>统一采用<br className="show-on-mobile"/>先进的<span style={{ fontWeight: 300 }}>消息段</span>(MS)</h3>
            <p className='text-left mb-8'>IamAI 会自动根据平台的消息事件内容生成对应的消息段，方便插件/模型开发者使用和调度。</p>
            <div>
              <div className={styles.optimization}>
                <div style={{ fontSize: '.9rem' }} className="leading-8">
                  <code>{`Adapter kook received: Event<message>: "Hi! Kook!!"`}</code>
                  <br/>
                  <code>{`Adapter cqhttp received: Event<message>: "Hi! CQHTTP!!"`}</code>
                </div>
              </div>
              <ArrowRightIcon width="1.2em" className="text-neutral-400 rotate-90 my-6 mx-auto" />
              <div className={styles.optimization}>
                <div style={{ fontSize: '.9rem' }} className="leading-8">
                  <code>{`KookMessage(KookMessageSegment<text>:"Hi! Kook!!")`}</code>
                  <br/>
                  <code>{`CQHTTPMessage(CQHTTPMessageSegment<text>:"Hi! CQHTTP!!")`}</code>
                </div>
              </div>
            </div>
          </div>
        ...
        <Feature index={2} id="highlighting-card" href="/docs/guide/syntax-highlighting">
          <h3>完善的 <br className="show-on-mobile"/>类型提示。</h3>
          <p>IamAI 使用[pydantic]()为开发提供强有力的类型提示支持，你甚至可以不需要查看文档，类型提示就是文档！</p>
        </Feature>
        ...
        <Feature index={10} large>
          ...
          <p className="subtitle"><Link className="!no-underline" href="/docs">开始使用IamAI →</Link></p>
        </Feature>
      </Features>
    </div>
  </div>
