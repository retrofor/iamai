...
```yaml filename="config.yml" showLineNumbers {5-7,14}
account:
  uin: 机器人QQ号
  password: '机器人密码'
...
```python filename="plugins/hallo-cqhttp.py" showLineNumbers
...
```python filename="plugins/test-cqhttp.py" showLineNumbers
from iamai import Plugin
