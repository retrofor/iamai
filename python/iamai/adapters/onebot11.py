...
class OneBot11Adapter(ModeSwitchingAdapterMiddleware):
    ...
    def __init__(self, runtime: "Runtime", config: dict[str, Any] | None = None) -> None:
        ...
        path_event = self.config.get("path_event")
        path_api = self.config.get("path_api")
        self.path_event = str(self.path if path_event is None else path_event)
        self.path_api = str(self.path if path_api is None else path_api)
